...
public class BistroMonitor implements Runnable {
	...
	public void run() {
		while (true) {
			try {
				isClosed = !server.isBistroOpen(new IsBistroOpenRequest(LocalDateTime.now()));
	...
	private void notifyAboutOrder() {
		...
		for (Map.Entry<String, String> entry : contacts.entrySet()) {
			...
			System.out.println(" Order " + orderNumber + " has been notified through contact.");
			if (contact.contains("@"))
				sendEmailOrderInTwoHours(orderNumber, contact);
			ServerUI.updateInScreen("for contact: " + contact + "\n your order " + orderNumber + " is in 2 hours");
		}
	...
	private void checkExpiredOrders() {
		...
		for (Map.Entry<String, String> entry : contacts.entrySet()) {
			String orderNumber = entry.getKey();
			String contact = entry.getValue();
			System.out.println(" Order " + orderNumber + " has expired before seating.");
			if (contact.contains("@"))
				sendEmailOrderExpired(orderNumber, contact);
			ServerUI.updateInScreen("for contact: " + contact + " Order " + orderNumber
	...
	/**
	 * A method that checks orders that entered through the waiting lists and didn't
	 * arrive yet
	 */
		...
		for (Map.Entry<Order, LocalDateTime> entry : pending.entrySet()) {
			...
			if (LocalDateTime.now().isAfter(addedTime.plusMinutes(15))) {
				...
				if (order.getContact().contains("@"))
					sendEmailOrderExpired(order.getOrderNumber(), order.getContact());
				ServerUI.updateInScreen("for contact: " + order.getContact() + " \n Seating time exceeded for order: "
						+ order.getOrderNumber() + " Please contact the bistro staff.");
				...
				for (Map.Entry<Table, Order> tableEntry : currentBistro.entrySet()) {
					if (tableEntry.getValue() != null
							&& tableEntry.getValue().getConfirmationCode().equals(order.getConfirmationCode())) {
	...
	private void checkOrdersAndAdvanceTime() {
		...
		for (Map.Entry<Table, Order> entry : currentBistro.entrySet()) {
			...
			if (!LocalDateTime.now().isBefore(twoHoursAfterSitting)) {
				...
				System.out.println(
						" Order " + order.getConfirmationCode() + " exceeded 2 hours at table " + table.getId());
				if (order.getContact().contains("@"))
					sendEmailTimeExceeded(order.getOrderNumber(), order.getContact());
				ServerUI.updateInScreen(
						"contact: " + order.getContact() + " Order " + order.getOrderNumber() + "\nYour bill is "
								+ (order.getSubscriberId().equals("0") ? BistroServer.BILL : BistroServer.BILL * 0.9)
								+ " NIS. Thank you for dining with us!"
	...
	private void trySeatFromWaitlist() {
		...
		for (Order wl : InAdvanceList) {
			...
			if(isClosed) {
				if(wl.getContact().contains("@"))
					sendEmailBistroClosed(wl.getOrderNumber(), wl.getContact());
				else {
					ServerUI.updateInScreen("for contact: " + wl.getContact() + " Order " + wl.getOrderNumber()
							+ "\nThe bistro is now closed. Please wrap up your visit. Thank you for dining with us!");
				}
			...
			System.out.println("Tried to seat order from inAdvance, confCode: " + wl.getConfirmationCode()
					+ ", result is: " + res);
			if (res != -1) {
				toRemove.add(new WaitlistNode(wl));
				if (wl.getContact().contains("@"))
					sendTableReadyEmail(wl.getContact(), wl.getOrderNumber(), res);
				ServerUI.updateInScreen("for contact: " + wl.getContact() + "\n order number: " + wl.getOrderNumber()
						+ "\ntable number " + res
						+ " got available for you,please come to the Bistro within 15 minute from this massage");
		...
		for (Order wl2 : RegularList) {
			...
			if(isClosed) {
				if(wl2.getContact().contains("@"))
					sendEmailBistroClosed(wl2.getOrderNumber(), wl2.getContact());
				else {
					ServerUI.updateInScreen("for contact: " + wl2.getContact() + " Order " + wl2.getOrderNumber()
							+ "\nThe bistro is now closed. Please wrap up your visit. Thank you for dining with us!");
				}
			...
			System.out.println(
					"Tried to seat order from regular, confCode: " + wl2.getConfirmationCode() + ", result is: " + res);
			if (res != -1) {
				toRemove.add(new WaitlistNode(wl2));
				if (wl2.getContact().contains("@"))
					sendTableReadyEmail(wl2.getContact(), wl2.getOrderNumber(), res);
	...
	private int trySeatHelper(Order order, WaitingList waitlist) {
		...
		Map<String, Integer> guests_in_time = server
				.prepareGuestsInTimeList(new ShowTakenSlotsRequest(guests, OrderDateTime), false);
		for (Order o : currentBistro.values()) {
			if (o != null) {
	...
	public void sendTableReadyEmail(String recipientEmail, String orderNum, int tableNumber) {
		...
		String body = String.format("Hello,\n\n" + "Good news! Your table is now ready.\n\n"
				...
				+ "The Bistro Team", tableNumber, orderNum);
