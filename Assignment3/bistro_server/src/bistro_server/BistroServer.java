...
public class BistroServer extends AbstractServer {
	...
	public List<String> getLiveState(Request r) {
		...
		for (Map.Entry<Table, Order> entry : currentBistro.entrySet()) {
			...
			if (t.isTaken() && o != null) {
				seatedSb.append("Table ").append(t.getId()).append(" (Max ").append(t.getCapacity()).append("): ")
						.append("Order #").append(o.getOrderNumber()).append("\n");
			}
		}
	 ...
	 * @return the table ID if available, -1 if not enough tables, 0 if others can
	 *         be seated
	 */
	public synchronized int checkAvailability(List<Table> tables, Map<String, Integer> guests_in_time,
		...
		for (Entry<String, Integer> entry : guests_in_time.entrySet()) {
			...
			for (Table t : tables) {
				System.out.println("Checking table ID: " + t.getId() + " with capacity: " + t.getCapacity()
						+ " (Taken: " + t.isTaken() + ") for party size: " + entry.getValue());
	...
	/**
	 * * Handles a customer leaving the waitlist Searches by order number
	 * 
	...
	public String handleJoinWaitlist(Request r) {
		...
		if (tableId != -1) {
			waitlistOrder = addNewOrder(new WriteRequest(req.getOrderDateTime(), req.getNumberOfGuests(),
					req.getSubscriberId(), req.getContact()));
			...
			waitlistOrder.setSittingtime(LocalDateTime.now());
	 ...
	 * @param copyCurrentBistroState = whether to copy the current taken state of
	 *                               the tables
	 ...
	 * @param isJustArrived = 1 for just arrived waitlist, 0 for ordered in advance
	 *                      waitlist
	 */
	...
	public synchronized String reserveTableInAdvance(Request r) {
		...
		ShowTakenSlotsRequest slotReq = new ShowTakenSlotsRequest(Integer.parseInt(req.getNumberOfGuests()),
				req.getOrderDateTime(), before, after);
		...
		if (available != -1) {
			...
			while (!before.isAfter(until)) {
				slotReq = new ShowTakenSlotsRequest(Integer.parseInt(req.getNumberOfGuests()), requested.toString(),
						before, after);
	 ...
	 * @param isNotInDatabase indicates if the current request is not yet in the
	 *                        database
...
//		monitorThread.start();
	...
	private String getTableForOrder(Request r) {
		...
		if (orderDate.isBefore(LocalDateTime.now().minusMinutes(15))
				|| orderDate.isAfter(LocalDateTime.now().plusMinutes(15))) {
			return "Your reservation is for " + orderDate.format(DT_FMT).toString()
					+ " Please arrive within 15 minutes of your reservation time.";
		}
		Map<String, Integer> guests_in_time = prepareGuestsInTimeList(new ShowTakenSlotsRequest(number_of_guests, date),
				false);
		for (Order o : currentBistro.values()) {
			if (o != null) {
		...
		if (tableId != -1) {
			for (Table t : currentBistro.keySet()) {
				if (t.getId() == tableId) {
					...
					o.setSittingtime(LocalDateTime.now());
					System.out.println("Setting sitting time to current Bistro time: " + LocalDateTime.now().toString()
		...
		for (Entry<Table, Order> entry : currentBistro.entrySet()) {
			...
			if (order != null && order.getConfirmationCode().equals(confcode) && order.getSittingtime() != null) {
				...
				if (userType.equals("Not found")) {
					...
					return "Order closed successfully. As a subscriber, you get a 10% discount! Your final bill is "
							+ (BILL * 0.9) + " NIS. Thank you for dining with us!";
				}
			}
		}
